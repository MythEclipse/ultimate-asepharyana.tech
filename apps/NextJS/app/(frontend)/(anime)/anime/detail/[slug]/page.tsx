...
export default function DetailAnimePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: anime, error } = useSWR<AnimeData>(
    resolvedParams ? `${BaseUrl}/api/anime/detail/${resolvedParams.slug}` : null,
    fetcher
  );
  ...
  useEffect(() => {
    if (resolvedParams && typeof window !== "undefined") {
      ...
      setBookmarked(bookmarks.some((item: { slug: string }) => item.slug === resolvedParams.slug));
    }
  }, [resolvedParams]);
  ...
  const handleBookmark = () => {
    ...
    if (bookmarked) {
      bookmarks = bookmarks.filter((item: { slug: string }) => item.slug !== resolvedParams.slug);
    } else {
      bookmarks.push({
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {anime.data.episode_lists?.length > 0 ? (
                    anime.data.episode_lists.map((episode) => {
                      const episodeNumber =
                        episode.episode.match(/Episode (\d+)/)?.[1] || episode.episode;
                      return (
                        <Link
                          scroll
                          key={episode.slug}
