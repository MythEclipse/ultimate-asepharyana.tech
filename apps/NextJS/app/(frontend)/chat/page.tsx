...
export default function ChatClient() {
  ...
  const [status, setStatus] = useState({ connected: false, sending: false, uploading: false });
  ...
  useEffect(() => {
    ...
    ws.current.onmessage = (e) => {
      ...
      mutate(prev => prev?.some(m => m.id === message.id) ? prev : [...(prev || []), message], false);
    };
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (file) {
        ...
        const { url } = await fetch('/api/uploader', { method: 'POST', body: formData }).then(res => res.json());
      ...
      <Card>
        ...
        <div className='h-96 overflow-y-auto p-4 space-y-3'>
          {messages?.map((message) => (
            <MessageBubble key={message.id} message={message} isOwn={message.email === session?.user?.email} />
          ))}
          <div ref={endRef} />
        </div>
            ...
            <div className="flex items-center gap-2 h-full">
              ...
              <label
                ...
                className={`h-10 px-3 py-2 flex items-center justify-center rounded-md text-sm border ${
                  status.uploading ? 'text-gray-400 border-gray-400' : 'text-blue-500 border-blue-500 hover:bg-blue-50'
                } cursor-pointer`}
              >
  ...
  return (
    <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
      <div className={`flex items-start gap-3 max-w-[85%] ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        />
        <div className={`p-3 rounded-lg ${isOwn ? 'bg-blue-500 text-white' : 'bg-gray-100'}`}>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-medium">{isOwn ? 'You' : message.user}</span>
            <span className="text-xs px-2 text-gray-500">{format(message.timestamp, 'HH:mm')}</span>
          </div>
