...
export default function ChatClient() {
  ...
  const { data: messages = [], mutate } = useSWR<Message[]>('chat-messages', null, {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      // Rollback jika gagal
      mutate((prev = []) => prev.filter(msg => msg.id !== newMessage.id), false);
    }
    ...
    <div className="container mx-auto py-8 px-4 max-w-2xl">
      ...
      <Card>
        <div className="p-4 border-b">
          ...
          <div className="text-sm opacity-75 mt-1">User ID: {userId.slice(0, 8)}</div>
        </div>
          ...
          {messages.map((message) => (
            ...
            >
              <div className="chat-header">
                {message.userId === userId ? 'You' : `User ${message.userId.slice(0, 6)}`}
                <time className="text-xs opacity-50 ml-2">
                  {new Date(message.timestamp).toLocaleTimeString()}
                </time>
              </div>
              <div className={`chat-bubble ${
                message.userId === userId ? 'chat-bubble-primary' : 'chat-bubble-secondary'
              }`}>
                {message.text}
              </div>
            </div>
