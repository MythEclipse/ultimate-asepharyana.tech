...
const TypeLabel = ({ type, badge }: { type?: string; badge?: string }) => {
  ...
  return (
    <span className={`absolute top-2 right-2 px-2 py-1 rounded-md text-white text-sm font-bold ${typeColors[label as keyof typeof typeColors] || 'bg-gray-500'}`}>
      {label}
    </span>
  )
}

export default function CardA({ title, description, imageUrl, linkUrl, type, badge, loading }: CardProps) {
    ...
    <Link href={linkUrl || '#'}>
      <div className="cursor-pointer transform transition-transform duration-300 hover:scale-105 hover:rotate-1 active:scale-95">
        <ShadcnCard className="w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl bg-white dark:bg-black overflow-hidden transform transition-transform duration-300 hover:shadow-2xl text-blue-500 border border-blue-500 rounded-xl shadow-lg shadow-blue-500/50 hover:bg-blue-500 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50 hover:ring-4 hover:ring-gradient-to-r hover:from-blue-400 hover:via-purple-500 hover:to-pink-500">
          ...
          <div className="p-4">
            <h3 className={`text-xs sm:text-sm md:text-lg font-bold text-black dark:text-gray-200 truncate ${loading ? 'bg-gray-300 animate-pulse h-5 w-3/4 rounded' : ''}`}>
              {(!loading || !isLoading) && title}
            </h3>
            {description && (
              <p className={`text-sm sm:text-sm md:text-base text-gray-600 dark:text-gray-400 mt-2 ${loading ? 'bg-gray-300 animate-pulse h-4 w-full rounded' : ''}`}>
