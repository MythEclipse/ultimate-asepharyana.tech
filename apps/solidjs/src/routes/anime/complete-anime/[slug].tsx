...
async function fetchCompleteAnime(page: string): Promise<ListResponse> {
    return httpClient.fetchJson<ListResponse>(`/api/anime/complete-anime/${page}`);
}

export default function CompleteAnimePage() {
    ...
    return (
        <>
            <Title>Complete Anime - Page {page()} | Asepharyana</Title>
            <main class="p-4 md:p-8 min-h-screen bg-background text-foreground">
                <div class="max-w-7xl mx-auto">
                    <h1 class="text-3xl font-bold mb-8 bg-gradient-to-r from-green-600 to-purple-600 bg-clip-text text-transparent">
                        Complete Anime
                    </h1>
                                ...
                                <>
                                                ...
                                                >
                                                    ...
                                                    <div class="absolute bottom-0 left-0 right-0 p-3">
                                                        ...
                                                        <Show when={item.episode_count}>
                                                            <span class="text-xs text-green-300">{item.episode_count} eps</span>
                                                        </Show>
                                                    </div>
                                                </A>
                                            )}
                                    ...
                                    <div class="flex justify-center gap-2">
                                        <Show when={parseInt(page()) > 1}>
                                            <A href={`/anime/complete-anime/${parseInt(page()) - 1}`} class="px-4 py-2 rounded-lg bg-card border border-border hover:bg-accent">Previous</A>
                                        </Show>
                                        <span class="px-4 py-2">Page {page()}</span>
                                        <A href={`/anime/complete-anime/${parseInt(page()) + 1}`} class="px-4 py-2 rounded-lg bg-card border border-border hover:bg-accent">Next</A>
                                    </div>
                                </>
                            )}
                        </Show>
