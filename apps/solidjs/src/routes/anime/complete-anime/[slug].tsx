...
async function fetchCompleteAnime(page: string): Promise<ListResponse> {
  return httpClient.fetchJson<ListResponse>(
    `/api/anime/complete-anime/${page}`,
  );
}
                ...
                <>
                  ...
                  <div class="flex justify-center gap-2">
                    <Show when={parseInt(page()) > 1}>
                      <A
                        href={`/anime/complete-anime/${parseInt(page()) - 1}`}
                        class="px-4 py-2 rounded-lg bg-card border border-border hover:bg-accent"
                      >
                        Previous
                      </A>
                    </Show>
