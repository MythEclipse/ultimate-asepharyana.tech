...
async function fetchKomikDetail(slug: string): Promise<KomikDetailResponse> {
    return httpClient.fetchJson<KomikDetailResponse>(`/api/komik/detail?komik_id=${encodeURIComponent(slug)}`);
}
...
export default function KomikDetailPage() {
    ...
    const [data] = createResource(() => enabled() ? params.slug : null, fetchKomikDetail);
                                            ...
                                            <div class="flex flex-col md:flex-row gap-8">
                                                ...
                                                <div class="flex-1 space-y-6">
                                                    <div>
                                                        ...
                                                        <p class="text-muted-foreground">by {komik().author}</p>
                                                    </div>

                                                    {/* Stats Grid */}
                                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                        <div class="glass-subtle rounded-xl p-4 text-center">
                                                            <div class="text-muted-foreground text-sm mb-1">Status</div>
                                                            <div class="font-semibold text-primary">{komik().status}</div>
                                                        </div>
                                                        <div class="glass-subtle rounded-xl p-4 text-center">
                                                            <div class="text-muted-foreground text-sm mb-1">Type</div>
                                                            <div class="font-semibold">{komik().type}</div>
                                                        </div>
                                                        <div class="glass-subtle rounded-xl p-4 text-center">
                                                            <div class="text-muted-foreground text-sm mb-1">Chapters</div>
                                                            <div class="font-semibold">{komik().total_chapter}</div>
                                                        </div>
                                                        <div class="glass-subtle rounded-xl p-4 text-center">
                                                            <div class="text-muted-foreground text-sm mb-1">Updated</div>
                                                            <div class="font-semibold text-sm">{komik().updated_on || '-'}</div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="glass-subtle rounded-xl p-5">
                                                        ...
                                                        <p class="text-muted-foreground leading-relaxed">{komik().description}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
