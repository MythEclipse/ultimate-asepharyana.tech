...
import { getDb, users, quizNotifications, eq, and, desc } from '@asepharyana/services';

// Handler: Get notification list
export async function handleNotificationListSync(ws: any, data: WSMessage<NotificationListSyncPayload>) {
  ...
  const [unreadResult] = await db
    .select()
    .from(quizNotifications)
    .where(and(eq(quizNotifications.userId, userId), eq(quizNotifications.isRead, 0)));
...
export async function handleNotificationMarkRead(ws: any, data: WSMessage<NotificationMarkReadPayload>) {
  ...
  for (const notifId of notificationIds) {
    await db
      ...
      .set({ isRead: 1 })
      .where(and(eq(quizNotifications.id, notifId), eq(quizNotifications.userId, userId)));
  }
...
export async function handleNotificationMarkAllRead(ws: any, data: WSMessage<NotificationMarkAllReadPayload>) {
...
export async function handleNotificationDelete(ws: any, data: WSMessage<NotificationDeletePayload>) {
  const { userId, notificationId } = data.payload;
  const db = getDb();
  
  await db
    .delete(quizNotifications)
    .where(and(eq(quizNotifications.id, notificationId), eq(quizNotifications.userId, userId)));
