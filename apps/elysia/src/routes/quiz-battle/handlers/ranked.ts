...
function getTierFromMMR(mmr: number): {
  tier: RankedTier;
  ...
  if (mmr < 1000) {
    ...
    return {
      tier: 'Bronze',
      division: Math.min(4, Math.max(1, div)) as RankedDivision,
    };
  } else if (mmr < 1500) {
...
export async function updateRankedMMR(
  userId: string,
  opponentMMR: number,
  isWinner: boolean,
) {
  ...
  const promoted =
    (oldRank.tier !== newRank.tier &&
      TIER_THRESHOLDS[newRank.tier].min > TIER_THRESHOLDS[oldRank.tier].min) ||
  ...
  const mmrChangePayload = {
    oldMMR,
    newMMR,
    mmrChange: mmrChange,
    change: mmrChange, // Keep for backwards compatibility
    tier: newRank.tier,
