...
export const quizBattleWS = new Elysia({ prefix: '/api/quiz' })
  .ws('/battle', {
    ...
    message(ws, rawMessage: unknown) {
      try {
        const message: WSMessage = typeof rawMessage === 'string' ? JSON.parse(rawMessage) : rawMessage as WSMessage;

        console.log(`[WS] Received message type: ${message.type}`);

        // Get session ID from WeakMap
        const sessionId = sessionIds.get(ws.raw as ServerWebSocket<WSData>) || Math.random().toString(36).substring(2, 15);
