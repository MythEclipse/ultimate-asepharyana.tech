                ...
                {move || chapter_data.get().flatten().map(|data| view! {
                        ...
                        <div class="glass border-b border-border/50 py-6 px-10 shadow-2xl">
                            <div class="max-w-7xl mx-auto flex items-center justify-between">
                                <div class="flex items-center gap-8">
                                    ...
                                    <div class="space-y-1">
                                        <p class="text-[10px] font-black uppercase tracking-[0.2em] text-primary">"Now Reading"</p>
                                        <h1 class="font-black italic text-lg truncate max-w-md uppercase tracking-tight">{data.title.clone()}</h1>
                                    </div>
                                </div>
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 rounded-2xl bg-primary/20 flex items-center justify-center text-xl shadow-2xl">"👁️"</div>
                                </div>
                            </div>
                        </div>
                    </nav>
                    ...
                    >
                        ...
                        <div class="fixed inset-0 pointer-events-none z-0">
                            <div class="absolute inset-0 bg-gradient-to-b from-primary/5 via-transparent to-transparent opacity-20" />
                        </div>
                            ...
                            {data.images.iter().enumerate().map(|(i, img)| view! {
                                <div class="relative w-full group/img overflow-hidden transition-all duration-1000">
                                    ...
                                    <div class="absolute top-1/2 -right-12 group-hover/img:right-6 transition-all duration-700 opacity-0 group-hover/img:opacity-100">
                                        <div class="glass-card px-3 py-1.5 rounded-xl border border-border/50 text-[10px] font-black text-foreground/40">
                                            {format!("{:02}", i + 1)}
                                        </div>
                                    </div>
                                </div>
                    ...
                    <footer class=move || format!("fixed bottom-0 left-0 right-0 z-[60] transition-all duration-700 ease-in-out {}", if show_controls.get() { "translate-y-0 opacity-100" } else { "translate-y-full opacity-0" })>
                        <div class="glass border-t border-border/50 p-8 shadow-[0_-50px_100px_rgba(0,0,0,0.2)]">
                            <div class="max-w-3xl mx-auto flex items-center justify-between gap-8">
                                <div class="flex-1">
                                    {if !data.prev_chapter_id.is_empty() {
                                        view! {
                                            <a href=format!("/komik/read/{}", urlencoding::encode(&data.prev_chapter_id)) class="group flex items-center gap-4 px-10 py-5 rounded-[2rem] glass-subtle border border-border/50 hover:border-primary/40 transition-all font-black uppercase text-[10px] tracking-widest hover:-translate-x-2">
                                        ...
                                        view! { <div class="px-10 py-5 rounded-[2rem] bg-muted/50 opacity-20 border border-dashed border-border/20" /> }.into_view()
                                    }}
                                </div>

                                <div class="px-8 flex flex-col items-center">
                                    <span class="text-[8px] font-black uppercase tracking-[0.4em] text-primary opacity-60">"Scroll"</span>
                                    <div class="w-1 h-8 bg-gradient-to-b from-primary to-transparent rounded-full animate-pulse mt-2" />
                                </div>

                                <div class="flex-1 flex justify-end">
                                    {if !data.next_chapter_id.is_empty() {
                                        view! {
                                            <a href=format!("/komik/read/{}", urlencoding::encode(&data.next_chapter_id)) class="group flex items-center gap-4 px-10 py-5 rounded-[2rem] bg-primary text-white hover:scale-105 active:scale-95 transition-all font-black uppercase text-[10px] tracking-widest shadow-2xl shadow-primary/20 hover:translate-x-2">
                                                "Next Chapter"
                                                <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        view! { <div class="px-10 py-5 rounded-[2rem] bg-primary/20 opacity-50 border border-dashed border-primary/30" /> }.into_view()
                                    }}
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="py-40 text-center space-y-8 animate-fade-in relative overflow-hidden">
                        <div class="absolute inset-0 bg-gradient-to-t from-primary/5 to-transparent pointer-events-none" />
                        <div class="relative z-10 flex flex-col items-center gap-6">
                            <div class="w-20 h-20 rounded-[2.5rem] glass border border-border/50 flex items-center justify-center text-3xl shadow-2xl shadow-primary/10">
                                "🎉"
                            </div>
                            <div class="space-y-2">
                            ...
                            <div class="h-px w-20 bg-gradient-to-r from-transparent via-border/20 to-transparent" />
