...
export function PaginationControls({
  ...
  const { current_page, last_visible_page, has_next_page, has_previous_page, next_page, previous_page } = pagination;
  ...
  const renderButton = (
    ...
    const content = (
      <>
        {direction === 'prev' && <ChevronLeft className={variant === 'compact' ? 'w-4 h-4' : 'w-5 h-5'} />}
        {direction === 'prev' ? 'Previous' : 'Next'}
        {direction === 'next' && <ChevronRight className={variant === 'compact' ? 'w-4 h-4' : 'w-5 h-5'} />}
      </>
    );
    ...
    if (onPageChange) {
      return (
        <button onClick={() => onPageChange(pageNumber)} className={buttonClass}>
          {content}
        </button>
      );
    }
  ...
  return (
    <div className={cn('flex flex-wrap gap-4 justify-between items-center', className)}>
      <div className="flex gap-4">
        {renderButton('prev', has_previous_page, previous_page || current_page - 1)}
