...
export interface CardSystemProps {
  ...
  skeleton?:
    | boolean
    | Omit<React.ComponentProps<typeof CardSkeleton>, 'variant'>;
  ...
  ({
    ...
    if (loading || skeleton) {
      ...
      const skeletonVariant =
        variant === '3d' || variant === 'mini-3d' ? '3d' : 'default';
      return (
        <CardSkeleton
          ...
          <CardContainer className="inter-var cursor-pointer">
            ...
            >
              ...
              {image?.show !== false && image?.src && (
                <CardItem
                  translateZ={isMini ? '100' : '20'}
                  className={cn('w-full mt-4', isMini && 'h-[60%]')}
                >
                  <CardImage
                    src={image.src}
    ...
    const cardContent = (
      ...
      >
        ...
        <div className="p-4">
          {title && (
            <h3
              className={cn('text-lg font-semibold truncate', titleClassName)}
            >
              {title}
            </h3>
          )}
          {description && (
            <p
              className={cn(
                'text-sm text-gray-600 line-clamp-2 mt-1 dark:text-gray-400',
                descriptionClassName,
              )}
            >
              {description}
