...
function Anime2PageClient({ initialData, initialError }: Anime2PageClientProps) {
  const { data, error, isLoading, mutate } = useSWR<HomeData>(
    `/api/anime2/`,
    fetcher,
    { revalidateOnFocus: false, fallbackData: initialData || undefined, refreshInterval: 60000 }
  );
  ...
  if (displayError || !displayData || displayData.status !== 'Ok' || !displayData.data) {
    return (
