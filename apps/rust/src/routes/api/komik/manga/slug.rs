...
pub async fn list(
  ...
  let result = fetch_and_parse_manga(&app_state.browser_pool, &url).await;
...
async fn fetch_and_parse_manga(
  ...
  let start = Instant::now();
  info!("Fetching and parsing manga from {}", url);
  ...
  info!("Fetched and parsed manga in {:?}", start.elapsed());
