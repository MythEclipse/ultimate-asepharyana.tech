...
pub async fn list(
  ...
  match fetch_and_parse_manhua_list(&Arc::new(TokioMutex::new(())), &url, page).await {
    Ok((data, pagination)) => {
...
async fn fetch_and_parse_manhua_list(
  ...
  if let Some(entry) = HTML_CACHE.get(url) {
    if entry.1.elapsed() < CACHE_TTL {
      ...
      return task::spawn_blocking(move ||
        parse_manhua_list_document(&Html::parse_document(&entry.0), page)
      ).await?;
    } else {
  ...
  task::spawn_blocking(move ||
    parse_manhua_list_document(&Html::parse_document(&html), page)
  ).await?
}
