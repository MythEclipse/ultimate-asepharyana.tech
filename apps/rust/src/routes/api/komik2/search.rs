...
#[allow(dead_code)]
pub const ENDPOINT_PATH: &str = "/api/komik2/search";
#[allow(dead_code)]
pub const ENDPOINT_DESCRIPTION: &str = "Searches for komik2 based on query parameters.";
#[allow(dead_code)]
pub const ENDPOINT_TAG: &str = "komik2";
#[allow(dead_code)]
pub const OPERATION_ID: &str = "komik2_search";
#[allow(dead_code)]
...
pub struct SearchQuery {
  /// Search query string to filter komik2 results
...
#[utoipa::path(
    ...
    path = "/api/komik2/search",
    tag = "komik2",
    operation_id = "komik2_search",
    responses(
        (status = 200, description = "Searches for komik2 based on query parameters.", body = SearchResponse),
...
pub async fn search(
  ...
  info!("Starting komik2 search for query: '{}', page: {}", query, page);

  let cache_key = format!("komik2:search:{}:{}", query, page);
  ...
  let (data, pagination) = fetch_and_parse_search(&url, page).await.map_err(|e| {
    error!(
      "Failed to process komik2 search for query: '{}', page: {} after {:?}, error: {:?}",
      query,
      page,
      start_time.elapsed(),
      e
  ...
  info!(
    "Successfully processed komik2 search for query: '{}', page: {} in {:?}",
    query,
    page,
    total_duration
  );
