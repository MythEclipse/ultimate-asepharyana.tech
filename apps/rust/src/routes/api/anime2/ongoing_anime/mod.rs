// Handler for GET /api/anime2/ongoing-anime/{slug}.
// Fetches a paginated list of ongoing anime from alqanime.net and returns JSON with anime data and pagination info.
...
#[utoipa::path(
    get,
    path = "/ongoing-anime/{slug}",
    params(
        ("slug" = String, Path, description = "Page number or slug for ongoing anime")
    ),
    responses(
        (status = 200, description = "List of ongoing anime", body = OngoingAnimeResponse)
    )
)]
