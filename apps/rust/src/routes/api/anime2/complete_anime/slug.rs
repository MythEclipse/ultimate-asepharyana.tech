...
pub const ENDPOINT_DESCRIPTION: &str = "Handles GET requests for the anime2/complete-anime/slug endpoint.";
...
pub async fn slug(Path(slug): Path<String>) -> impl IntoResponse {
    match fetch_html(&format!("https://alqanime.net/advanced-search/page/{}/?status=completed&order=update", slug)).await {
        Ok(html) => {
