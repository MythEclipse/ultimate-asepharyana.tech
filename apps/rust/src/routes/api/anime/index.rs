...
pub async fn anime(State(app_state): State<Arc<AppState>>) -> Result<
  impl IntoResponse,
  (StatusCode, String)
> {
  ...
  if let Some(json_data_string) = cached_response {
    ...
    let anime_response: AnimeResponse = serde_json::from_str(&json_data_string).map_err(|e| {
  ...
  match fetch_anime_data().await {
    Ok(data) => {
      ...
      info!("Successfully processed anime index in {:?}", start_time.elapsed());
