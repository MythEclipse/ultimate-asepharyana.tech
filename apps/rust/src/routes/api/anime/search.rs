...
async fn fetch_and_parse_search(url: &str, query: &str) -> Result<SearchResponse, Box<dyn std::error::Error>> {
    ...
    let genre_selector = Selector::parse(".set b:contains(\"Genres\") ~ a").unwrap();
...
fn parse_pagination(document: &Html, _query: &str) -> Pagination {
    ...
    Pagination {
        ...
        has_previous_page,
        previous_page: if has_previous_page { Some(page_num - 1) } else { None },
    }
}
