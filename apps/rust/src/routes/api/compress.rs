...
fn parse_size_param(
    ...
    if s.ends_with('%') {
        ...
        if trimmed_s.is_empty() {
            tracing::error!(
                "parse_size_param: Numeric value missing for percentage for input '{}'",
                size_param
            );
        ...
        if trimmed_s.is_empty() {
            tracing::error!(
                "parse_size_param: Numeric value missing for MB for input '{}'",
                size_param
            );
        ...
        if trimmed_s.is_empty() {
            tracing::error!(
                "parse_size_param: Numeric value missing for KB for input '{}'",
                size_param
            );
        ...
        tracing::error!(
            "parse_size_param: Invalid size format for input '{}'",
            size_param
        );
...
async fn compress_image(
        ...
        if let Ok(metadata) = fs::metadata(&cache_path).await {
            if let Ok(modified) = metadata.modified()?.duration_since(UNIX_EPOCH) {
                ...
                if now.as_millis() - modified.as_millis() < (*CACHE_EXPIRY as u128) {
                    ...
                    if cached.is_empty() {
                        tracing::warn!(
                            "Cached image file is empty, invalidating cache and re-compressing."
                        );
                        fs::remove_file(&cache_path).await?; // Invalidate empty cache
                    } else {
                        let size_reduction = (((buffer.len() - cached.len()) as f64)
                            / (buffer.len() as f64))
                            * 100.0;
    ...
    for i in 0..8 {
        tracing::info!(
            "Attempt {} for image compression, current quality: {}",
            i + 1,
            quality
        );
    ...
    tracing::info!(
        "Image compression finished after all attempts for cache key: {}",
        cache_key
    );
...
async fn compress_video(
        ...
        if let Ok(metadata) = fs::metadata(&cache_path).await {
            if let Ok(modified) = metadata.modified()?.duration_since(UNIX_EPOCH) {
                ...
                if now.as_millis() - modified.as_millis() < (*CACHE_EXPIRY as u128) {
                    ...
                    if cached.is_empty() {
                        tracing::warn!(
                            "Cached video file is empty, invalidating cache and re-compressing."
                        );
                        fs::remove_file(&cache_path).await?; // Invalidate empty cache
                    } else {
                        let size_reduction = (((buffer.len() - cached.len()) as f64)
    ...
    let caps = duration_regex
        .captures(&stderr)
        .ok_or("Could not find duration in ffmpeg output")?;
    ...
    if !status_pass1.status.success() {
        return Err(format!(
            ...
            String::from_utf8_lossy(&status_pass1.stderr)
        )
        .into());
    }
    ...
    if !status_pass2.status.success() {
        return Err(format!(
            ...
            String::from_utf8_lossy(&status_pass2.stderr)
        )
        .into());
    }
    ...
    if ext == "mp4" || ext == "mov" || ext == "avi" {
        ...
        if let Some(k) = kind {
            ...
            if !k.mime_type().starts_with("video/") {
                tracing::error!(
                    "Buffer content type mismatch. Expected video/*, got: {}",
                    k.mime_type()
                );
                return Err(
                    format!("Invalid content type for video file: {}", k.mime_type()).into(),
                );
            }
        } else {
