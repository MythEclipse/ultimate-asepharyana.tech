            ...
            match msg {
                Message::Text(text) => {
                    ...
                    if let Ok(chat_message) = serde_json::from_str::<ChatMessage>(&text) {
                        ...
                        let mut clients = recv_task_state.clients.lock().unwrap();
                        drop(clients); // Explicitly drop the mutex guard before the await
                        chat_service::broadcast_message(message_to_save, &mut recv_task_state.clients.lock().unwrap()).await;
                    } else {
