...
start_server() {
    ...
    while [ $retry_count -lt $max_retries ] && [ "$server_ready" = false ]; do
        ...
        # Checking root endpoint of anime
        if curl -s "$BASE_URL/api/anime" > /dev/null 2>&1; then
             server_ready=true
...
echo -e "\n${YELLOW}=== Testing Anime Endpoints ===${NC}"

# 1. Index
test_api "GET" "/api/anime" "200" "Get Anime Index"

# 2. Latest
test_api "GET" "/api/anime/latest?page=1" "200" "Get Latest Anime"

# Extract a slug from latest for further tests
LATEST_RES=$(curl -s "${BASE_URL}/api/anime/latest?page=1")
...
# 3. Detail
test_api "GET" "/api/anime/detail/$SLUG" "200" "Get Anime Detail ($SLUG)"

# 4. Search
test_api "GET" "/api/anime/search?q=naruto&page=1" "200" "Search Anime (naruto)"

# 5. Genres
test_api "GET" "/api/anime/genres" "200" "Get Genres List"

# 6. Filter (might need params)
test_api "GET" "/api/anime/filter?page=1" "200" "Filter Endpoint"
...
test_api "GET" "/api/anime/ongoing-anime/$SLUG" "200" "Ongoing Anime Info ($SLUG)"

# 8. Complete Anime 
test_api "GET" "/api/anime/complete-anime/$SLUG" "200" "Complete Anime Info ($SLUG)"

# 9. Genre Slug (valid genre needed, e.g., 'action')
test_api "GET" "/api/anime/genre/action?page=1" "200" "Get Genre 'action'"
